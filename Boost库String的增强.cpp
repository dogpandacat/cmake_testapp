...
vector<string> Split2(const string& s, char delimiter) {
//    把s放到istringstream里面去
//    通过std::getline从istringstream中读取，就好像是用getline从控制台读取一样
    vector<string> result;
    std::istringstream input(s, ios_base::in);
    string item;
...
/**
 * 官方文档：https://www.boost.org/doc/libs/1_83_0/doc/html/string_algo/quickref.html#id-1.3.3.6.2
 *
 * @param argc
 * @param argv
 * @return
...
int main(int argc, char* argv[])
{
    ...
    //token_compress_on是压缩相邻令牌的意思
    split( SplitVec, str1, is_any_of("-*"), token_compress_on ); // SplitVec == { "hello abc","ABC","aBc goodbye" }

    cout << "C++中借助boost库实现string的split，结果 = ";
    for(auto& item : SplitVec)
    {
        cout << item << " ";
    }
    cout << endl;
    ...
    //继续测试使用自己写的Split2函数
    std::string s = "hello world! how are you today?";
    char delimiter = ' ';
    std::vector<std::string> tokens = Split2(s, delimiter);
    cout << "Split2函数，tokens.size = " << tokens.size() << endl;
